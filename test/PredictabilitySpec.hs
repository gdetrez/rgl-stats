...
module PredictabilitySpec where
...
import Predictability -- SUT

-- Shortcut for a quiet shelly
sh = shellyNoDir . silently
...
spec = do
  ...
  describe "getLexicon" $ do
    context "Toy grammar LexiconEng" $ do
      it "get the lexicon for the V category" $
        liftM sort (sh $ getLexicon "gf" "test/LexiconEng.gf" "V" 3)
          `shouldReturn` [["eat","ate","eaten"],["love","loved","loved"],["pray","prayed","prayed"]]

      it "gets the lexicon for the N category" $
        liftM sort (sh $ getLexicon "gf" "test/LexiconEng.gf" "N" 2)
  ...
  describe "computeConcrete" $ do
    context "with the toy morphology" $ do
      ...
      it "uses mkN with 1 argument" $
        sh (computeConcrete "gf" "test/ParadigmEng.gf" "mkN" ["\"mouse\""])
          `shouldReturn` ["mouse","mouses"]
      it "uses mkN with 2 argument" $
        sh (computeConcrete "gf" "test/ParadigmEng.gf" "mkN" ["\"mouse\"","\"mices\""])
          `shouldReturn` ["mouse","mices"]

      it "uses mkV with 1 argument" $
        sh (computeConcrete "gf" "test/ParadigmEng.gf" "mkV" ["\"eat\""])
          `shouldReturn` ["eat", "eated","eated"]
      it "uses mkV with 2 argument" $
        sh (computeConcrete "gf" "test/ParadigmEng.gf" "mkV" ["\"eat\"","\"ate\""])
          `shouldReturn` ["eat", "ate","ate"]
      it "uses mkV with 3 argument" $
        sh (computeConcrete "gf" "test/ParadigmEng.gf" "mkV" ["\"eat\"","\"ate\"","\"eaten\""])
  ...
  context "Toy experiment with english nouns" $ do
    let experiment = Experiment { title = "English nouns"
                                , lexicon = "test/LexiconEng.gf"
                                , category = "N"
                                , nforms        = 2
                                , morphology = "test/ParadigmEng.gf"
                                , smartparadigm = "mkN"
                                , setup = \[cat,cats] -> [[esc cat],[esc cat,esc cats]] }
    describe "word costs" $ do
        it "cat has cost 1" $
          sh (wordCost "gf" experiment ["cat", "cats"]) `shouldReturn` 1
        it "dog has cost 1" $
          sh (wordCost "gf" experiment ["dog", "dogs"]) `shouldReturn` 1
        it "mouse has cost 2" $
          sh (wordCost "gf" experiment ["mouse", "mices"]) `shouldReturn` 2
  ...
  context "Toy experiment with english verbs" $ do
    ...
    describe "word costs" $ do
        it "pray has cost 1" $
          sh (wordCost "gf" experiment ["pray", "prayed", "prayed"]) `shouldReturn` 1
        it "love has cost 2" $
          sh (wordCost "gf" experiment ["loved", "loved", "loved"]) `shouldReturn` 2
        it "eat has cost 3" $
          sh (wordCost "gf" experiment ["eat","ate","eaten"]) `shouldReturn` 3
    describe "runExperiment" $ do
      let report = ExperimentReport { experiment   = "English verbs"
                                    , entries      = 3
                                    , meanCost     = 2
